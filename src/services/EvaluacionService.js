class EvaluacionService {
    ...
    async iniciar(datos) {
        ...
        if (!total_preguntas || isNaN(total_preguntas) || total_preguntas <= 0) {
            throw new Error(
                "El total de preguntas debe ser un número mayor a 0"
            );
        }
    ...
    async obtenerPorId(id_evaluacion) {
        ...
        const evaluacion =
            await this.evaluacionRepository.obtenerPorId(id_evaluacion);
        ...
        const detalle =
            await this.evaluacionRepository.obtenerDetallePorEvaluacion(
                id_evaluacion
            );
    ...
    async obtenerHistorial(id_usuario) {
        ...
        return await this.evaluacionRepository.obtenerHistorialPorUsuario(
            Number(id_usuario)
        );
    }
    ...
    async registrarRespuesta(id_evaluacion, datos) {
        ...
        const respondidas =
            await this.evaluacionRepository.contarRespuestasRegistradas(
                id_evaluacion
            );

        if (respondidas >= evaluacion.total_preguntas) {
            throw new Error(
                "Ya se respondieron todas las preguntas de esta evaluación"
            );
        }
        ...
        const yaRespondida =
            await this.evaluacionRepository.yaRespondioPregunta(
                id_evaluacion,
                id_pregunta
            );
        ...
        if (yaRespondida) {
            throw new Error(
                "Esta pregunta ya fue respondida en esta evaluación"
            );
        }
        ...
        const respuestaCorrecta =
            await this.evaluacionRepository.obtenerRespuestaCorrecta(
                id_pregunta
            );
        ...
        return await this.evaluacionRepository.registrarRespuesta({
            ...
            tiempo_usado: tiempo_usado || null
        });
    }
    ...
    async finalizar(id_evaluacion) {
        ...
        const evaluacion =
            await this.evaluacionRepository.obtenerPorId(
                id_evaluacion
            );
        ...
        const correctas =
            await this.evaluacionRepository.contarRespuestasCorrectas(
                id_evaluacion
            );
        ...
        // Al actualizar puntaje_obtenido,
        // el trigger MySQL actualiza ranking automáticamente.
        return await this.evaluacionRepository.actualizarPuntaje(
            id_evaluacion,
            puntaje
        );
    }
    ...
    async obtenerRankingPorArea(id_area) {
        ...
        return await this.evaluacionRepository.obtenerRankingPorArea(
            Number(id_area)
        );
    }
